...
def process(input, mask, anchors):
    ...
    box_xy += grid
    box_xy *= int(YOLOV5_CFG["net_size"]/grid_h)
...
def filter_boxes(boxes, box_confidences, box_class_probs):
    ...
    _box_pos = np.where(box_confidences >= YOLOV5_CFG["obj_thresh"])
    ...
    _class_pos = np.where(class_max_score >= YOLOV5_CFG["obj_thresh"])
...
def nms_boxes(boxes, scores):
    ...
    while order.size > 0:
        ...
        inds = np.where(ovr <= YOLOV5_CFG["nms_thresh"])[0]
        order = order[inds + 1]
    keep = np.array(keep)
    return keep


def process_yolov5_output(input_data):
...
def draw(image, boxes, scores, classes):
    ...
    for box, score, cl in zip(boxes, scores, classes): # type: ignore
        ...
        cv2.putText(
            img=image,
            text='{0} {1:.2f}'.format(YOLOV5_CFG["classes"][cl], score),
            org=(top, left - 6),
