...
def fast_nms_numpy(box_thre, coef_thre, class_thre):
    ...
    idx = idx[:, :YOLACT_CFG['top_k']]
    class_thre = class_thre[:, :YOLACT_CFG['top_k']]
    ...
    keep = (iou_max <= YOLACT_CFG['nms_iou_thre'])
    ...
    idx = idx[:YOLACT_CFG['max_detections']]
    class_nms = class_nms[:YOLACT_CFG['max_detections']]
...
def nms_numpy(class_pred, box_pred, coef_pred, proto_out, anchors):
    ...
    keep = (class_p_max > YOLACT_CFG['nms_score_thre'])
...
def after_nms_numpy(
    ...
    if YOLACT_CFG and YOLACT_CFG['visual_thre'] > 0:
        keep = class_p >= YOLACT_CFG['visual_thre']
        if not keep.any():
            return None, None, None, None
