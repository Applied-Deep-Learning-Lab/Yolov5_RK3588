...
class OrangePi():
    def __init__(self):
        ...
        self._cam = Cam(source=config.SOURCE, lock=self._lock, q_in=self._q_show, q_out=self._q_pre)
        self._yolov5 = [Yolov5(proc=i, lock=self._lock, q_in=self._q_pre, q_out=self._q_outs, core=self._cores[i%3]) for i in range(config.INF_PROC)]
        ...
        self._bytetracker = Process(target=bytetracker_draw, args=(self._lock, self._q_post, self._q_show), daemon=True)
