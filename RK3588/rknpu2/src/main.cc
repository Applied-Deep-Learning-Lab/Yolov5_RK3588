...
static int get_count_classes(char* path)
{
  FILE* fp;
  int classes_count = 0;
  char c;
...
int main(int argc, char** argv)
{
  ...
  if (argc != 4) {
    printf("Usage: %s <rknn model> <jpg> <txt> \n", argv[0]);
    return -1;
  }
  ...
  post_process(argv[3], (int8_t*)outputs[0].buf, (int8_t*)outputs[1].buf, (int8_t*)outputs[2].buf, height, width,
